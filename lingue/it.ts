...
const it = {
  ...
  legendFree:     "Libera", legendFreeDesc: "Puoi prenotarla subito.",
  legendInUse:    (t: string): string => `In uso — Turno in corso, fine alle ${t}.`,
  legendPrev:     "Camera — Turno precedente — Indica chi aveva lo slot prima di te.",
  legendOos:      "Fuori servizio — Puoi prenotarla comunque, a tuo rischio.",
  ...
  // ─── Le voci nuove: sale, prenotazioni, notifiche ─────────────────────────
  //
  // Vengono dal disegno "Organic": la dashboard che dice quanto manca al TUO
  // turno, le sale guardate tutte insieme, l'elenco unico delle prenotazioni,
  // lo storico delle notifiche.
  accessoDirezione: "Digita 1935 al posto della camera per l'accesso della Direzione.",
  terminaAlle: (ora: string): string => `termina alle ${ora}`,
  prossimoTurno: "Il prossimo turno",
  turniLiberi: (n: number): string => n === 1 ? "1 turno libero" : `${n} turni liberi`,
  altroGiorno: "Altro giorno",
  tuttaLaSettimana: "tutta la settimana",
  tuoTurno: "Il tuo turno",
  mostraNotte: (n: number, da: string, a: string): string => `Mostra i ${n} turni notturni (${da} – ${a})`,
  nascondiNotte: "Nascondi i turni notturni",
  quotaConferma: (usati: number, tot: number): string =>
    `Con questo usi ${usati} dei ${tot} turni di questa settimana.`,
  quotaRiparte: "Il conteggio riparte lunedì.",
  navSale: "Sale",
  navPrenotazioni: "Le mie prenotazioni",
  notifiche: "Notifiche",
  saleAdesso: "Le sale, adesso",
  saleNessunLimite: "nessun limite settimanale",
  saleNonDisponibili: "Le sale non rispondono. Riprova più tardi.",
  occupataDalle: (ora: string): string => `occupata dalle ${ora}`,
  prenotaFascia: "Prenota una fascia",
  prenotaDalle: (ora: string): string => `Prenota dalle ${ora}`,
  vediProgramma: "Vedi il programma",
  servelaCamera: "Serve il numero di camera per vedere le tue prenotazioni.",
  storico: "Storico",
  storicoVuoto: "Ancora niente nello storico.",
  serataAperta: "serata aperta",
  serataPrivata: "serata privata",
  notificheIntro: "Ti avvisiamo poco prima che inizi il turno, e quando passa all'asciugatrice.",
  nessunaNotifica: "Nessuna notifica",
  nessunaNotificaBody: "Qui restano i promemoria ricevuti su questo dispositivo.",
  svuotaStorico: "Svuota lo storico",
  notificheLocali: "Lo storico è salvato su questo dispositivo: cambiando telefono non ti segue.",
  adesso: "ora",
  ieri: "ieri",
};
